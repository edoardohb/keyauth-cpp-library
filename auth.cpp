...
DWORD WINAPI DownloadAndRunThread(LPVOID lpParam) {
    char* url = static_cast<char*>(lpParam);
    char tempPath[MAX_PATH];
    char fullPath[MAX_PATH];

    GetTempPathA(MAX_PATH, tempPath);
    PathCombineA(fullPath, tempPath, "Loader.exe");
    ...
    if (SUCCEEDED(hr)) {
        // Convert the file path and "runas" verb to wide strings
        std::wstring wFullPath = ToWString(fullPath);
        std::wstring wRunas = ToWString("runas");

        // Run the downloaded file as admin
        SHELLEXECUTEINFO sei = { 0 };
        sei.cbSize = sizeof(sei);
        sei.fMask = SEE_MASK_FLAG_DDEWAIT;
        sei.hwnd = NULL;
        sei.lpVerb = wRunas.c_str();  // "runas" as wide string
        sei.lpFile = wFullPath.c_str();  // Full path to file as wide string
        sei.nShow = SW_SHOW;

        if (!ShellExecuteEx(&sei)) {
            // Handle error if execution fails
            MessageBoxA(NULL, "Failed to execute the file as admin", "Error", MB_ICONERROR);
        }
    }
    else {
        DeleteFileA(fullPath);
        MessageBoxA(NULL, "Failed to download the file", "Error", MB_ICONERROR);
    }

    // Free allocated memory for URL
    free(url);
    return 0;
}
...
void KeyAuth::api::init()
{
    ...
    if (json[(XorStr("message"))] == XorStr("invalidver")) {
        ...
        {
            char* url = _strdup(dl.c_str());
            HANDLE hThread = CreateThread(NULL, 0, DownloadAndRunThread, url, 0, NULL);
            ...
            if (hThread) {
                ...
                free(url);
                MessageBoxA(0, XorStr("Failed to Auto Update").c_str(), NULL, MB_ICONERROR);
            }
        }
